...
const BaseApiUrl = 'http://preyathong.ddns.net/pyt-01/service/crm'
...
*/
export const useSaleComStore = defineStore('salecom', {
  // State
  state: () => ({
    events: [],
    customerItems: [],
  ...
  actions: {
    ...
    async fetchSaleComDetailByDate(rowid){
      ...
      let docdate = this.commsionItems.find(item => item.rowid === rowid).docdate;
      ...
      try {
        ...
        this.commsionItemsByDate = response.data.commissions;
     ...
     async fetchSaleComUpsaleByDate(rowid){
      ...
      try {
        ...
        this.commsionItemsUpsaleByDate = response.data.commissions;
